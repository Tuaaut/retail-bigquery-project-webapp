-- back compat for old kwarg name
    ...
    merge into `bq-test-query`.`analytics_dev`.`fct_order_items` as DBT_INTERNAL_DEST
        using (
...
with item_scope as (
  ...
  from `bq-test-query`.`analytics_dev`.`int_order_item_enriched`
    ...
    where order_date >= (select coalesce(max(order_date), date('1900-01-01')) from `bq-test-query`.`analytics_dev`.`fct_order_items`)
